...
int store = 0;
  double value = ceil((double)size/blockSize);// 6/2 = 3 or 6/5 = 2 cuz we actually need 2 blocks to store all 6 data uh
   ...
   if(freespaceHMethod()){//run freespace to check got space or not
    ...
    int k = 0;
      for(int j = 0; j < blocksRequired; j++){
        ...
        printf("freed and bitmap inside %d \n" , freed[j]);
         bitmap[freed[j]] = 0; 
       //   printf("print the calcutions for map %d\n" ,freed[0] + blocksRequired);
        // printf("bitmap[j] %d  and j is count %d\n" , bitmap[i] , i);
      }
...
int checkFuncNM(){//read csv and store everything into struct d[]
